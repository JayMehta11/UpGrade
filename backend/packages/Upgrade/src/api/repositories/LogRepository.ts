...
export class LogRepository extends Repository<Log> {
  ...
  public async deleteByMetricId(metricKey: string): Promise<any> {
    const queryRepo = this.manager.getRepository(Query);
  ...
  public async getMetricUniquifierData(
    ...
    const metricsRepository = this.manager.getRepository(Metric);
  ...
  > {
    const experimentRepo = this.manager.getRepository(ExperimentRepository);
  ...
  public async analysis(query: Query): Promise<any> {
    ...
    if (operationType === OPERATION_TYPES.PERCENTAGE) {
      ...
      if (unitOfAssignment === 'within-subjects') {
        ...
        const withinSubjectPercentQuery = this.createQueryBuilder()
          .select([
      ...
      if (unitOfAssignment === 'within-subjects') {
        ...
        const conditionOrLevelId = isFactorialExperiment ? 'levelId' : 'conditionId';
        if (operationType === OPERATION_TYPES.MEDIAN || operationType === OPERATION_TYPES.MODE) {
          withinSubjectExecuteQuery = this.manager
            .createQueryBuilder()
            .select([
              `subquery."${conditionOrLevelId}"`,
  ...
  private getCommonAnalyticQuery(
    ...
    const experimentRepo = this.manager.getRepository(Experiment);
